...
comm.Scatterv([checker,(0,(IUB-Istart)*(S),(IUB-Istart)*(S),(IUB-Istart)*(S),(IUB-Istart)*(S),(IUB-Istart)*(S),(IUB-Istart)*(S)),\
        (0,0,(IUB-Istart)*(S),2*(IUB-Istart)*(S),3*(IUB-Istart)*(S),4*(IUB-Istart)*(S),5*(IUB-Istart)*(S)),MPI.BOOL],checkerpiece)
        ...
        for s in xrange(S):
            for i in xrange(Istart,IUB):
                try:
                    ...
                    checkerpiece=np.reshape(checkerpiece[:,np.newaxis],(IUB-Istart,S))
        ...
        for s in xrange(S):
            for i in xrange(Istart,IUB):
                try:
                    ...
                    np.reshape(checkerpiece,(IUB-Istart,S))
                    checkerpiece[i-Istart,s]=False
                    np.reshape(checkerpiece, (IUB-Istart)*(S))
                    print "Failure at:", Sinputs[s], "cohorts,",i,"countries",sig,"slope"
                    #comm.Send(checkerpiece,dest=0)
        ...
        for s in xrange(S):
            for i in xrange(Istart,IUB):
                try:
                    ...
                    checkerpiece=np.reshape(checkerpiece,(IUB-Istart,S))
                    checkerpiece[i-Istart,s]=True
                    checkerpiece=nnp.reshape(checkerpiece, (IUB-Istart)*(S))
                    print "Success at:", Sinputs[s], "cohorts,",i,"countries",sig,"slope"
                    #comm.Send(checkerpiece,dest=0)
                except:
                    checkerpiece=np.reshape(checkerpiece,(IUB-Istart,S))
...
comm.Gatherv(checkerpiece,[checker,(0,(IUB-Istart)*(S),(IUB-Istart)*(S),(IUB-Istart)*(S),(IUB-Istart)*(S),(IUB-Istart)*(S),(IUB-Istart)*(S)),\
        (0,0,(IUB-Istart)*(S),2*(IUB-Istart)*(S),3*(IUB-Istart)*(S),4*(IUB-Istart)*(S),5*(IUB-Istart)*(S)),MPI.BOOL])

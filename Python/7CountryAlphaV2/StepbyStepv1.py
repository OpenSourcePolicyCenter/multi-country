...
def plotDemographics(index, years, name):
	...
	for y in xrange(len(years)):
		...
		if yeartograph <= T:
			plt.plot(xrange(S+1), N_matrix[index,:,yeartograph])
		else:
...
def getOtherVariables(assets, kf):
	...
	, assets[:,:-1]) - assets[:,1:]
        ...
        if np.any(k<0):
            print "WARNING! INFEASIBLE VALUE IN k!"
            Feasible=False
            print "THESE ARE THE COORDINATES OF INFEASIBLE ANSWERS:"
            print np.argwhere(k<0)

        if np.any(n<0):
            print "WARNING! INFEASIBLE VALUE IN n!"
            Feasible=False
            print "THESE ARE THE COORDINATES OF INFEASIBLE ANSWERS:"
        ...
        if np.any(y<0):
            print "WARNING! INFEASIBLE VALUE IN y!"
            Feasible=False
            print "THESE ARE THE COORDINATES OF INFEASIBLE ANSWERS:"
        ...
        if np.any(r<0):
            print "WARNING! INFEASIBLE VALUE IN r!"
            Feasible=False
            print "THESE ARE THE COORDINATES OF INFEASIBLE ANSWERS:"
        ...
        if np.any(w<0):
            print "WARNING! INFEASIBLE VALUE IN w!"
            Feasible=False
            print "THESE ARE THE COORDINATES OF INFEASIBLE ANSWERS:"
        ...
        if np.any(c_vec<0):
            print "WARNING! INFEASIBLE VALUE IN c_vec!"
            Feasible=False
            print "THESE ARE THE COORDINATES OF INFEASIBLE ANSWERS:"
...
def find_optimal_starting_consumptions(c_1, wpath_chunk, rpath_chunk, epath_chunk, starting_assets, current_s):
	...
	Euler = np.ravel(assets_path[:,-1])
        ...
        if c_path.any()<0:
            print "WARNING! INFEASIBLE VALUE IN c_path!"
            Feasible=False
            #print "THESE ARE THE COORDINATES OF INFEASIBLE ANSWERS:"
            #print np.argwhere(Kpath<0)
            Euler=np.ones(Countries)*9999.
...
def get_prices(Kpath, kf_tpath):
	...
	Kdpath=Kpath-kf_tpath
        ...
        if np.any(Kpath<0):
            print "WARNING! INFEASIBLE VALUE IN Kpath!"
            Feasible=False
            print "THESE ARE THE COORDINATES OF INFEASIBLE ANSWERS:"
            print np.argwhere(Kpath<0)
...
def get_wpath1_rpath1(w_path0, r_path0, starting_assets):
	...
	w_path1, r_path1, ypath1 = get_prices(Kpath,kfpath)
        ...
        if np.any(w_path1<0):
            print "WARNING! INFEASIBLE VALUE IN w_path1!"
            Feasible=False
            print "THESE ARE THE COORDINATES OF INFEASIBLE ANSWERS:"
            print np.argwhere(w_path1<0)

        if np.any(r_path1<0):
            print "WARNING! INFEASIBLE VALUE IN r_path1!"
            Feasible=False
            print "THESE ARE THE COORDINATES OF INFEASIBLE ANSWERS:"
            print np.argwhere(r_path1<0)


        if np.any(ypath1<0):
            print "WARNING! INFEASIBLE VALUE IN y_path1!"
            Feasible=False
            print "THESE ARE THE COORDINATES OF INFEASIBLE ANSWERS:"
            print np.argwhere(ypath1<0)


        if np.any(Cpath<0):
            print "WARNING! INFEASIBLE VALUE IN Cpath!"
            Feasible=False
            print "THESE ARE THE COORDINATES OF INFEASIBLE ANSWERS:"
            print np.argwhere(Cpath<0)

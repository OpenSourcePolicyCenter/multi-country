...
\section{Add Children and Leisure Decision}
	...
	If we stationarize \eqref{eq_3elldef}, \eqref{eq_3cKdef}, \eqref{eq_3Gamdef} and \eqref{eq_3cKdef}, we can add them to the list of other behavioral equations from previous sections above to get the following.
	...
	\begin{align}
		...
		& {\hat c^K_{ist}}^{-\sigma} - \beta \left(\hat c^K_{i,s+1,t+1} e^{g^A}\right)^{-\sigma}(1+r_{1,t+1}-\delta) = 0; \forall i,s \\ 
		& r_{it} - r_{1t} = 0; \forall i>1 \\
		& \sum_{i=1}^I \hat k^f_{it} \left( \sum_{s=1}^S \hat N_{ist} \right) = 0
	\end{align}
